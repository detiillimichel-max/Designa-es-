...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function baixarPdfDiscursos(index = null) { try { const indices = index === null ? escalaAtual.map((_, i) => i) : [index]; indices.forEach((itemIndex, ordem) => { const salvar = () => criarPdfDiscurso(itemIndex).save(`discurso-${inputDataInicio.value || 'sem-data'}-semana-${escalaAtual[itemIndex].semana}.pdf`); if (ordem) setTimeout(salvar, ordem * 350); else salvar(); }); mostrarStatus(`${indices.length} PDF${indices.length === 1 ? '' : 's'} de discurso preparado${indices.length === 1 ? '' : 's'}.`); } catch (err) { mostrarStatus(err.message, 'error'); } }
